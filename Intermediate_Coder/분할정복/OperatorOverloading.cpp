...
/// 위 코드를 보고 당황스런 표정을 짓고 있을 불쌍한 컴퓨터를 위해 우리가 할 수 있는 일은 Student형 자료형을 정의할 때, 작다(< )의 정의를 같이 정의해 주는 것이다.
...
/// }
/// 
/// 2단계와의 차이점은 딱 두 가지다. if문에 있는 함수호출 부분이 s1.smaller(s2)에서 s1<s2로 바뀐 것과, struct 구문 내의 함수(메소드)이름이 operator<로 바뀐 것이다.
/// 
/// 3단계의 operator<라는 이름의 함수(메소드)와 2단계의 smaller라는 이름의 함수는 역할이 똑같으나 호출 방식이 조금 다른 것 뿐이라는 것을 인지할 필요가 있다.
/// smaller를 호출하기 위해서는 s1.smaller(s2) 처럼 (호출하는 주체).(함수이름)으로 호출할 수 있었지만, operator<를 호출할 때는, s1.operator<(s2)가 아니라, s1<s2라고 써야 제대로 호출이 된다는 것이다.
